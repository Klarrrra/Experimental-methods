...
#The last one has the best fit (which makes sense as it has most parameters)
```
...
#Jonathans is the best model


BIC(m0, m1, m2, m_Jakob, m_Jonathan) #lower is better
#acording to BIC m2 is the best model
...
```{r}
model1 <- lm(Aggression ~ Computer_Games, data = child)
model2 <- lm(Aggression ~ Computer_Games + Television, child)
model3 <- lm(Aggression ~  Computer_Games + Television + Diet, child)
...
anova(model1, model2)
#going from  model 1 to 2 is significantly better

#comparing model 2 to 3
anova(model2, model3)
#going from  model 1 to 2 is not significantly better 

#compare model 1 to 3
anova(model1, model3)
#going from  model 1 to 3 is significantly better

#Due to ANOVA = number 2 is best

#Q: could I have just compared them all in one line?
...
```{r}
sex_death <- read_csv("/Users/marko/Documents/Sex&Death.csv")
...
anova_nofemale <- aov(Longevity ~ None0 + Pregnant1 + Pregnant8 + Virgin1 + Virgin8, data = sex_death)

#why cant it take Longevity?
view(anova_nofemale)
