...
```{r library}
personality <- read.csv("NEW_CogSciPersonalityTest2019.csv")
knitr::opts_knit$set(root.dir='/Users/marko/Downloads/NEW_CogSciPersonalityTest2019.csv.numbers') #setting the working directory
...
rplot <- ggplot(personality, aes(x = personality$gender, y = personality$ratio2d4d, colour = personality$gender)) +
  theme_minimal() +
...
```{r visualisation}
#1 and 2
plotMusicCola <- ggplot(personality, aes(x = taste_cola, y = hours_music_per_week)) +
  theme_minimal() +
    labs(x = "cola", y = "music") 
...
#yes - option S has a big outlier 

#3 same with removed outlier 

filtered <- personality %>% filter(hours_music_per_week<100)
ggplot(filtered, aes(x = taste_cola, y = hours_music_per_week)) +
  geom_boxplot(width = 0.5) +
  ggtitle("Box Plot with mean")
...
rplot +
  ...
  geom_errorbar(aes(ymin = mean(personality$ratio2d4d) - sd(personality$ratio2d4d), ymax = mean(personality$ratio2d4d) + sd(personality$ratio2d4d)), width = 0.1, color = 'black') +
...
```{r}
#error bars: standard deviation, standard error, confidens interval
...
#1, 2 and 3
ggplot(personality, aes(x = choose_rand_num)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("choose random number") +
  stat_function(fun = dnorm, args = list(mean = mean(personality$choose_rand_num, na.rm = TRUE), sd = sd(personality$choose_rand_num, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()
#its definitely not normally destributed

#4
ggplot(personality, aes(x = shoesize)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("shoesize") +
  stat_function(fun = dnorm, args = list(mean = mean(personality$shoesize, na.rm = TRUE), sd = sd(personality$shoesize, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()

#it might be - but here it apperas binomial becouse of genders
#I am not sure of the Shapiro-Wilk test
...
# one for men and one for woman
males <- personality %>% filter(gender == "male") #only males
females <- personality %>% filter(gender == "female") #only males

ggplot(males, aes(x = shoesize)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("shoesize") +
  stat_function(fun = dnorm, args = list(mean = mean(males$shoesize, na.rm = TRUE), sd = sd(males$shoesize, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()


ggplot(females, aes(x = shoesize)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("shoesize") +
  stat_function(fun = dnorm, args = list(mean = mean(females$shoesize, na.rm = TRUE), sd = sd(females$shoesize, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()
...
```{r}
#1
#I dont completely understand the datatransformation (why to transform to what)
New <- personality %>% mutate(hours_log = log(hours_music_per_week), hours_sqrt = sqrt(hours_music_per_week), hours_1 = 1/hours_music_per_week)

New

#2

ggplot(New, aes(x = hours_log)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25, color= 'black', fill = 'white') +
  ggtitle("Log trasnformed") +
  stat_function(fun = dnorm, args = list(mean = mean(mdf$hours_log, na.rm = TRUE), sd = sd(mdf$hours_log, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()

#the log transformation seems best

ggplot(New, aes(x = hours_sqrt)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25, color= 'black', fill = 'white') +
  ggtitle("Log trasnformed") +
  stat_function(fun = dnorm, args = list(mean = mean(mdf$hours_log, na.rm = TRUE), sd = sd(mdf$hours_log, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()
...
ggplot(New, aes(x = hours_music_per_week)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25, color= 'black', fill = 'white') +
  ggtitle("Log trasnformed") +
  stat_function(fun = dnorm, args = list(mean = mean(mdf$hours_music_per_week, na.rm = TRUE), sd = sd(mdf$hours_music_per_week, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()
...
var <- ggplot(personality, aes(x = tongue_twist, y = balloon_balance)) +
  geom_point() 

#how could I do this for first men and then woman?

plot <- ggplot(personality, aes(x = personality$balloon_balance, y = personality$tongue_twist, fill = balloon_balance)) +
  facet_wrap(~native_Danish) + 
  geom_bar(stat = 'summary', fun.y = mean, width = 0.25)+
  theme_minimal() +
  labs(x = "balloon", y = "tongue") +
  ggtitle("Faceting")


ggsave("myggplot.png")  # saves the last plot.
ggsave("mystoredggplot.png", plot=plot)  # save a stored ggplot
