...
view(kiki$id)
#well - I am not sure how to do this 
class(id)
...
```{r}
#not sure if this will work as I have NAN
```
...
#this doesnt work -maybe it has something to do with the id thing i didnt know how to do
summary(m2)
```
...
#result = 10% chance of "jagged" if you get a B

#log odds into probability for estimate of going from letter B to letter K
boot::inv.logit(-2.178216 + 3.290078) 
#result = 75% chance of "jagged" if you get a K
...
#I want to make a model that checks if this hypothesis is true:
#to hear "i" will make you think of small objekt and "o" of big object
#it is a repaeted meassure design so I will use GLMM
view(kiki)
mymodel <- lme4::glmer(size ~ vowel + (1+vowel|id), kiki, family = binomial)
summary(mymodel)
...
```{r}
inv.logit(-0.3795)
#40% chance of "small" if you got an "0"


inv.logit(-0.3795 + 0.6308)
#56%  chance of "small" if you get an "i"

#so we see a tendensy, but not as strong as the k = jagged and B=curvy
```
...
my_model <- glm(size ~ vowel + word + consonant, kiki, family = binomial)
vif(my_model)
